...
RESULTS_FILE="test-results-download.txt"

# Limpiar archivo de resultados
echo "=== RESULTADOS DE PRUEBAS DE DESCARGA ===" > $RESULTS_FILE
...
echo -e "${BLUE}🚀 INICIANDO BATERÍA DE PRUEBAS DE DESCARGA${NC}"
...
response=$(curl -s -w "\n%{http_code}" -X GET "$SERVER_URL/files/$(head -n1 uploaded_files.txt)")
http_code=$(echo "$response" | tail -n1)
