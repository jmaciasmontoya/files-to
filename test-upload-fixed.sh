...
TEST_DIR="test-files"
RESULTS_FILE="test-results-fixed.txt"

# Limpiar archivo de resultados
echo "=== RESULTADOS DE PRUEBAS DE SUBIDA (CORREGIDO) ===" > $RESULTS_FILE
...
test_upload() {
    ...
    if [ "$http_code" = "201" ] || [ "$http_code" = "200" ]; then
        if echo "$response_body" | grep -q '"success":true'; then
            ...
            echo "$uuid" >> uploaded_files_fixed.txt
            echo "UUID guardado: $uuid"
        else
...
echo -e "${BLUE}🚀 INICIANDO BATERÍA DE PRUEBAS DE SUBIDA (CORREGIDO)${NC}"
echo "Servidor: $SERVER_URL"
echo "Directorio de pruebas: $TEST_DIR"
echo ""

# Limpiar archivo de UUIDs subidos
> uploaded_files_fixed.txt
...
# 3. Archivo PDF válido (8MB - dentro del límite)
test_upload "$TEST_DIR/large-valid.pdf" "201" "PDF válido (8MB)"
...
echo "Archivos subidos exitosamente: $(wc -l < uploaded_files_fixed.txt)"
echo "Resultados guardados en: $RESULTS_FILE"
echo ""

# Mostrar UUIDs de archivos subidos para pruebas de descarga
if [ -s uploaded_files_fixed.txt ]; then
    echo -e "${GREEN}📁 Archivos subidos exitosamente:${NC}"
    cat uploaded_files_fixed.txt
    echo ""
fi
