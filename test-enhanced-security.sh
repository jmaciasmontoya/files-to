...
RESULTS_FILE="enhanced-security-test-results.txt"

# Limpiar archivo de resultados
echo "=== PRUEBAS DE SEGURIDAD MEJORADA ===" > $RESULTS_FILE
...
echo -e "${PURPLE}🛡️ INICIANDO PRUEBAS DE SEGURIDAD MEJORADA${NC}"
...
echo -e "${BLUE}🚦 Probando rate limiting...${NC}"
echo "Enviando 5 requests rápidas..."
...
for i in {1..5}; do
    ...
    if [ "$http_code" = "429" ]; then
        show_result "Rate Limiting" "PASS" "Rate limiting funcionando correctamente"
        break
    fi
    
    if [ $i -eq 5 ] && [ "$http_code" != "429" ]; then
        show_result "Rate Limiting" "FAIL" "Rate limiting no está funcionando"
    fi
done
...
# Resumen final
echo ""
echo -e "${PURPLE}📊 RESUMEN DE PRUEBAS DE SEGURIDAD MEJORADA${NC}"
echo "=================================================="
...
echo -e "${GREEN}✅ Pruebas de seguridad mejorada completadas${NC}"
